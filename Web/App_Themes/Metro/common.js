...
var theme_root = ___get_content_root("common.js");
var frm_stack = [], rss_arr = [];
var cur_color = 0, isIndex = false;
document.write("<link rel='stylesheet' type='text/css' href='" + theme_root + "site.css' />");
document.write("<link rel='stylesheet' type='text/css' id='style_color' href='" + theme_root + "color/" + (top.cur_color % 12) + ".css' />");
...
function ResizeFrame() {
    ...
    $(".body > .hscroll").css("height", height - 15);
    if ($(".page").length) height -= 35;
    // 兼容模式滚动条出来以后，显示正常情况被滚动条隐藏的部分内容。
    if ($.browser.msie && $.browser.version <= 7 && $(".index .body").height() < height)
        $(".index .body").css("padding-right", 17);
    else if ($.browser.msie && $.browser.version <= 7 && $(".index .body").height() > height)
        $(".index .body").css("padding-right", "auto");
    $(".index .body.hscroll").css("height", height);
    if ($(".create .body .hscroll").length <= 0) {
        $(".create .body.hscroll").css("height", height);
    }
    ResizeIFrame();
};
...
function InstallBodyTab() {
    ...
    $(".body > .hscroll > ul iframe").each(function () {
...
function InitLv1Menus() {
    ...
    $.each(menus, function (idx, mi) {
        ...
        //if (mi.cnt > 0) html += "<span class='cnt'>" + mi.cnt + "</span>";
        if (mi.rss) {
            html += "<span class='cnt'></span>";
            AddToRssArray(mi);
        }
        html += "</li>"
...
function InstallRssTicker() {
    setInterval(CheckUndoNumber, 60000); // 一分钟刷新一次未读数，根据实际性能情况调整
    setTimeout(CheckUndoNumber, 1000);
}

function CheckUndoNumber() {
    var temp_arr = [];
    var result = {};
    $.each(rss_arr, function () {
        temp_arr.push(this);
    });
    var getResultOne = function () {
        if (temp_arr.length) {
            var item = temp_arr.pop();
            if (result[item.rss]) {
                if (result[item.rss] > 99) {
                    $("#" + item.id + " .cnt").text("99+");
                }
                else if (result[item.rss] > 0) {
                    $("#" + item.id + " .cnt").text(result[item.rss]);
                }
                setTimeout(getResultOne, 30);
            }
            else {
                $.ajax(item.rss, { type: "POST", data: { uname: curr_uname }, complete: function (jqXHR, status) {
                    if (status == "success") {
                        var v = parseInt(jqXHR.responseText);
                        if (isNaN(v)) {
                            v = 0;
                        }
                        result[item.rss] = v;
...
function AvatarMenuToggle() {
    ...
    $(".portal > .head .info > .avatar, .index_page .avatar_index a").live("click", function () {
        if ($(":animated").length) return false;
        if (isAvatarFocus) {
...
function BuildMenu(menus, parent, lv) {
    ...
    $.each(menus, function (idx, mi) {
        ...
        if ($("body").hasClass("index_page")) {
            $(".portal > .menu").html("");
            elMI.addClass("icon");
            html = "<div class='img'></div> <span class='txt'>" + mi.text + "</span>";
            if (mi.rss) {
                html += " <span class='cnt'></span>";
            }
            elMI.html(html);
...
function ResizeIFrame() {
    try {
        var frm = $(".body > .hscroll > ul.cur iframe")[0];
        $(frm).height($(window).height() - 110);
        if (frm) {
            var height = Math.max(frm.contentWindow.document.documentElement.scrollHeight,
                frm.contentWindow.document.documentElement.offsetHeight,
                frm.contentWindow.document.body.scrollHeight);
            $(frm).height(height);
        }
    } catch (e) { }
}
